...
export async function POST(request) {
  ...
  if (!full) {
    if (user.wordwareCompleted || (user.wordwareStarted && Date.now() - new Date(user.createdAt).getTime() < 3 * 60 * 1000)) {
  ...
  if (full) {
    if (user.paidWordwareCompleted || (user.paidWordwareStarted && Date.now() - new Date(user.createdAt).getTime() < 3 * 60 * 1000)) {
...
*retweets: ${tweet.retweetCount ?? 0}, replies: ${tweet.replyCount ?? 0}, likes: ${tweet.likeCount ?? 0}, quotes: ${tweet.quoteCount ?? 0}, views: ${tweet.viewCount ?? 0}*`;
  ...
  const fullVersion = process.env.WORDWARE_FULL_VERSION || '^3.2';
  const roastVersion = process.env.WORDWARE_ROAST_VERSION || '^1.3';
  ...
  const existingAnalysis = user?.analysis;
